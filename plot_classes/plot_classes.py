...
def query_object_properties(class_domain, class_range):
  ...
  for i in xrange(len(class_range)):
    q_objprop_select = q_objprop_select+ "(onto:" + class_range[i] + ") "
  q_objprop_select = q_objprop_select + "}"
...
def dtp_box(graph, classname, out):
  ...
  out.write('%s[fontsize = "11"; shape=none;\n label = <<table border="1" cellborder="0" cellspacing="0">\n<tr><td port="port1" border="1" bgcolor="palegreen">%s</td></tr>' % (classname, classname))

  idx = 1
  for r in dtp:
    prop = r[0].split('#')[1]
    prop_type = r[1].split('#')[1]
    out.write('\n<tr><td port="port%i" align="left" bgcolor="white">%s: %s</td></tr>' % (idx+1, prop, prop_type))
  out.write('</table>>];')
...
def query_results_to_dot(graph, filename, classname):
  ...
  for i in xrange(no_classes-1):
	...
	prop_inverse = None
      ...
      if prop_inverse:
	out.write('%s->%s [dir="forward", arrowhead="none", arrowtail="normal", headlabel="%s 1", taillabel="1.. %s", labelfontsize="9", labeldistance="1"];\n' % (classname[i], prop_range, prop_inverse, prop))
      else:
	out.write('%s->%s [dir="forward", arrowhead="none", arrowtail="normal", headlabel="1", taillabel="1..", xlabel="%s", fontsize="9", labelfontsize="9", labeldistance="1"];\n' % (classname[i], prop_range, prop))
...
def main(argv):
  ...
  if class_names:
    ...
    ONTONAMESPACE = Namespace("http://www.semanticweb.org/ontology#");
